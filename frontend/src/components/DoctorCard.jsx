...
const DoctorCard = ({ name, speciality, experience, image }) => {
  return (
    <motion.div
      ...
      className="bg-white rounded-3xl p-5 shadow-lg shadow-gray-100 border border-gray-100 flex flex-col group cursor-pointer"
    >
      {/* Image Container */}
      ...
      {/* Info */}
      <div className="mb-3">
        <h3 className="text-lg font-black text-gray-800 tracking-tight leading-tight">{name}</h3>
        <p className="text-[#1e584a] font-bold text-sm mt-0.5">{speciality}</p>
      </div>

      {/* Experience Badge */}
      <div className="inline-flex items-center gap-1.5 bg-[#f0f9f6] text-[#22c55e] px-3 py-1 rounded-full w-fit mb-5">
        <Clock size={12} />
        <span className="text-[10px] font-black uppercase tracking-wider">{experience} Experience</span>
      </div>
